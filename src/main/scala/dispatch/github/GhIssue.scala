...
object GhIssue {
   implicit val formats = DefaultFormats
	...
	def get_issues(user: String, repo: String, page: Int, state: String, perPage: Int, accessToken: String): Promise[List[GhIssue]] = 
		get_issues(user, repo, Map("page" -> page.toString, "state" -> state, "per_page" -> perPage.toString, "access_token" -> accessToken))

	def get_issues(user: String, repo: String, page: Int, state: String, accessToken: String): Promise[List[GhIssue]] = 
		get_issues(user, repo, Map("page" -> page.toString, "state" -> state, "access_token" -> accessToken))

	def get_issues(user: String, repo: String, page: Int, accessToken: String): Promise[List[GhIssue]] = 
		get_issues(user, repo, Map("page" -> page.toString, "access_token" -> accessToken))

	def get_issues(user: String, repo: String, accessToken: String): Promise[List[GhIssue]] = 
		get_issues(user, repo, Map("access_token" -> accessToken))

	def get_issues(user: String, repo: String, page: Int, perPage: Int): Promise[List[GhIssue]] = 
		get_issues(user, repo, Map("page" -> page.toString, "per_page" -> perPage.toString))

	def get_issues(user: String, repo: String, page: Int): Promise[List[GhIssue]] = 
		get_issues(user, repo, Map("page" -> page.toString))

	def get_issues(user: String, repo: String, params: Map[String, String] = Map()): Promise[List[GhIssue]] = {
	...
	def create(user: String, repo: String, issue: GhIssueSummary, accessToken: String): Promise[GhIssue] = 
		create(user, repo, issue, Map("access_token" -> accessToken))

	def create(user: String, repo: String, issue: GhIssueSummary, params: Map[String, String] = Map()): Promise[GhIssue] = {
