...
object GhRepository {
   implicit val formats = DefaultFormats
	...
	def get_org_repos(org: String, access_token:String) : Future[List[GhRepository]] =
		get_org_repos(org, Map("access_token" -> access_token))

	def get_org_repos(org: String, page: Int, per_page: Int, access_token: String) : Future[List[GhRepository]] =
		get_org_repos(org, Map("page" -> page.toString, "per_page" -> per_page.toString, "access_token" -> access_token))

	def get_org_repos(org:String, params:Map[String, String] = Map()) : Future[List[GhRepository]] = {
	...
	def get_all_repos(access_token:String) : Future[List[GhRSimpleRepository]] =
		get_all_repos(Map("access_token" -> access_token))
   
	def get_all_repos(page: Int, per_page: Int, access_token: String) : Future[List[GhRSimpleRepository]] =
		get_all_repos(Map("page" -> page.toString, "per_page" -> per_page.toString, "access_token" -> access_token))

	def get_all_repos(params:Map[String, String] = Map()) : Future[List[GhRSimpleRepository]] = {
		val svc = GitHub.api_host / "repositories"
		val reposJson = Http(svc.secure <<? params OK as.lift.Json) 
		for (js <- reposJson) yield js.extract[List[GhRSimpleRepository]]
	}

	def get_branches(owner: String, repo: String, access_token:String) : Future[List[GhBranchSummary]] =
		get_branches(owner, repo, Map("access_token" -> access_token))

   def get_branches(owner: String, repo: String, page: Int, per_page: Int, access_token: String) : Future[List[GhBranchSummary]] =
		get_branches(owner, repo: String, Map("page" -> page.toString, "per_page" -> per_page.toString, "access_token" -> access_token))
	
   def get_branches(owner: String, repo: String, params:Map[String, String] = Map()) : Future[List[GhBranchSummary]] = {
